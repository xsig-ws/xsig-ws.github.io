...
function fluida_layout_meta_box() {
	global $post;
   	global $fluida_big;
	$options = $fluida_big['options'][0];
	$custom = ( get_post_custom( $post->ID ) ? get_post_custom( $post->ID ) : false );
	$layout = ( isset( $custom['_cryout_layout'][0] ) ? $custom['_cryout_layout'][0] : '0' );
	if ( empty($layout) && isset( $custom['_fluida_layout'][0] ) ) $layout = $custom['_fluida_layout'][0]; // back compat
    ?>
	<p>
    	...
    	<label id="cryout_layout_default">
            ...
            <span><img src="<?php echo esc_url( get_template_directory_uri() ) ?>/admin/images/0def.png" alt="<?php _e( 'Default Theme Layout', 'fluida' ); ?>" title="<?php _e( 'Default Theme Layout', 'fluida' ); ?>" /></span>
        </label>
	</p>
	<?php
} //fluida_layout_meta_box()
...
function fluida_save_custom_post_metadata() {
	...
	// Layout - Update
	delete_post_meta( $post->ID, '_fluida_layout', $layout ); // back compat cleanup
