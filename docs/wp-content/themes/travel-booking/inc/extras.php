...
function travel_booking_get_homepage_section(){
    ...
    if( $ed_search_bar && travel_booking_is_wte_advanced_search_active() ) array_push( $sections, 'sections/search' );
    
    if( is_active_sidebar( 'about' ) ) array_push( $sections, 'sections/about' );
    if( $ed_popular ) array_push( $sections, 'popular' );
    if( is_active_sidebar( 'cta-one' ) ) array_push( $sections, 'sections/cta-one' );
    if( $ed_feature ) array_push( $sections, 'featured-trip' );
    if( $ed_deal )array_push( $sections, 'deals' );
    if( $ed_destination )array_push( $sections, 'destination' );
    if( is_active_sidebar( 'cta-two' ) ) array_push( $sections, 'sections/cta-two' );
